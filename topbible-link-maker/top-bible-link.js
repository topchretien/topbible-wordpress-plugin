...
topbible_link_editor.backbone_modal.Application = Backbone.View.extend(
	{
		...
		initialize: function () {
			...
			_.bindAll( this, 'render', 'preserveFocus', 'closeModal', 'saveModal', 'saveModalRef', 'doNothing',
                             'bookSelect', 'chapterSelect', 'verseSelect', 'toVerseSelect' );
		...
		saveModal: function ( e ) {
			...
			if (topbible_link_editor.JSONresponse.length !== 0) {
				for (var i = 0; i < topbible_link_editor.JSONresponse.length; i++) {
					if (topbible_link_editor.JSONresponse[i]['selected'] === true) {
						textToInsert = textToInsert +
						topbible_link_editor.JSONresponse[i]['text']+
						' (<a class="button" href="' + 
						topbible_link_editor.JSONresponse[i]['url'] + '">' + 
						topbible_link_editor.JSONresponse[i]['ref'] + '</a>) ';
					}
				}
			}
			   ...
			   success: function(response) {
			      	...
			      	if (response.length !== 0) {
			      		...
			      		if (topbible_link_editor.JSONresponse.length !== 0) {
			      			...
			      			for (var i = 0; i < topbible_link_editor.JSONresponse.length; i++) {
			      				result = result +
			      				"<li><div><a href='#' title='Insérer texte + référence avec lien' onclick='topbible_link_editor.backbone_modal.__instance.insertLinkAndText("+i+")'><img src='"+topbible_link_editor.url+"/more.png'><img src='"+topbible_link_editor.url+"/plus.png'></a><a title='Insérer référence avec lien' href='#'onclick='topbible_link_editor.backbone_modal.__instance.insertLink("+i+")'><img src='"+topbible_link_editor.url+"/plus.png'></a></div><label><h3><input type='checkbox' onclick='topbible_link_editor.backbone_modal.__instance.clickOnSearchList(this,"+i+")'>" +
        ...
        insertLink: function(i) {
            var textToInsert = ' <a class="button" href="' +
                topbible_link_editor.JSONresponse[i]['url'] + '">' +
                topbible_link_editor.JSONresponse[i]['ref'] + '</a> ';
            tinymce.execCommand('mceInsertContent', false, textToInsert);
            this.closeModal();
        },
        insertLinkAndText: function(i) {
            var textToInsert = topbible_link_editor.JSONresponse[i]['text']+
                ' (<a class="button" href="' + 
                topbible_link_editor.JSONresponse[i]['url'] + '">' + 
                topbible_link_editor.JSONresponse[i]['ref'] + '</a>) ';
            tinymce.execCommand('mceInsertContent', false, textToInsert);
        ...
        chapterSelect: function() {
            ...
            if (chapter) {
                ...
                console.log(book, chapter, verses, options);
        ...
        bookSelect: function() {
            var book = parseInt(jQuery('#userBookSelect').val());
            console.log(jQuery('#userBookSelect').val(), book);
            if (book) {
                book--;
